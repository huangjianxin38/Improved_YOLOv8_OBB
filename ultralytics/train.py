...
import os
os.environ["WANDB_API_KEY"] = "API keys"
os.environ["WANDB_MODE"] = "offline"

if __name__ == "__main__":
    # 加载模型
    model = YOLO(r'ultralytics/cfg/models/v8/Improved_YOLOv8-OBB.yaml')  # 不使用预训练权重从头训练
    # model = YOLO(r'C:\Users\Huang\Desktop\YOLOv8-Magic-main\ultralytics-8.1.0\ultralytics\cfg\models\v8\obb\yolov8s-obb-slim-neck-p6-lsk.yaml').load(r"yolov8s-obb.pt")  # 使用预训练权重训练
    # 训练参数 ----------------------------------------------------------------------------------------------
    model.train(
        data=r"path_to_dataset",# 数据集的路径
        epochs=200,  # (int) 训练的周期数
        patience=None,  # (int) 等待无明显改善以进行早期停止的周期数
        ...
        workers=16,  # (int) 数据加载的工作线程数（每个DDP进程）
        ...
        warmup_epochs=5.0,  # (float) 预热周期（分数可用）
        ...
        copy_paste=0.05,  # (float) 分割复制-粘贴（概率）
    )
